...
function readingTimeFromFrontmatter(raw: unknown): number | null {
...
const fallbackReadingMinutes = ref<number | null>(null)

const readingTimeMinutes = computed(() => {
  const fm = readingTimeFromFrontmatter(frontmatter.value.readingTime)
  return fm ?? fallbackReadingMinutes.value
})

onMounted(() => {
  if (readingTimeFromFrontmatter(frontmatter.value.readingTime) != null) return
  const content = document.querySelector('.issue-post__content')
  if (!content) return
  const text = content.textContent || ''
  const wordCount = text.trim().length
  fallbackReadingMinutes.value = Math.max(1, Math.ceil(wordCount / 300))
})
</script>
  ...
  <article class="issue-post">
    ...
    <header class="issue-post__header">
      ...
      <div class="issue-post__meta">
        ...
        <template v-if="readingTimeMinutes != null">
          <span class="issue-post__meta-sep">·</span>
          <span>{{ readingTimeMinutes }} 分钟阅读</span>
        </template>
        <template v-if="frontmatter.issueUrl">
          <span class="issue-post__meta-sep">·</span>
          <a
