    ...
    <header class="issue-post__header">
      ...
      <ul v-if="labels.length" class="issue-post-labels" role="list" aria-label="标签">
        <li v-for="lb in labels" :key="lb.name" class="issue-post-label-item">
          <span class="issue-post-label">{{ lb.name }}</span>
        </li>
      </ul>
      ...
      <div class="issue-post__meta">
        <img class="issue-post__meta-avatar" :src="avatarUrl" width="24" height="24" :alt="ghUser" />
        ...
        <a v-if="frontmatter.issueUrl" :href="frontmatter.issueUrl" target="_blank" rel="noreferrer" class="issue-post__meta-link">
